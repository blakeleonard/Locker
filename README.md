...
Locker currently requires zero permissions, and doesn’t display advertisements or perform data collection.  Relatively minimal in terms of features, a user can compose, edit, save, and delete notes, and edit note names and passphrases.  Additionally, note size is currently limited to 200 lines.  Locker uses a key extension algorithm to transform a passphrase into a 256 Bit Key and then encrypts the note using AES (Advanced Encryption Standard).

Google Play Store:  [Locker](https://play.google.com/store/apps/details?id=net.leonardlabs.locker)
